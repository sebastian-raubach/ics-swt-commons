...
/**
 * {@link ParameterStore} is the central instance holding the {@link Parameter}s
...
{
    protected static final Map<Parameter, Object> STATE_PARAMETERS = new HashMap<Parameter, Object>();

    /**
     * Get a specific {@link Parameter} from the {@link ParameterStore}
     * 
     * @param key
     *            The {@link Parameter} identifier
    ...
    /**
     * Gets a specific {@link Parameter} from the {@link ParameterStore}. Makes
     * sure that it is in {@link String} form. This will, e.g., convert a
    ...
    {
        if (ParameterStore.get(key) == null)
            return null;
        else if (key.getType().equals(List.class))
            return CollectionUtils.joinList((List<?>) ParameterStore.get(key), ",");
        else
     ...
     *            The {@link Parameter} identifier
     * @param fallback
     *            The value that is returned if the requested {@link Parameter}
     *            doesn't have a value
     * @return The {@link Parameter} or <code>null</code>
     */
    public static String getAsString(Parameter key, String fallback)
    {
        if (ParameterStore.get(key) == null)
            return fallback;
        else
     ...
     *            The {@link Parameter} identifier
     * @param value
     *            The actual {@link Parameter} value
     * @return the previous value associated with key, or null if there was no
     *         mapping for the key. (A null return can also indicate that the
     ...
     *            The actual {@link Parameter} value
     * @param type
     *            The {@link Class} of this {@link Parameter}
     * @return the previous value associated with key, or null if there was no
     *         mapping for the key. (A null return can also indicate that the
     *         map previously associated null with the key)
    ...
    /**
     * Returns true if the {@link Parameter} store contains a mapping for the
     * specified key.
     * 
     * @param key
     *            The {@link Parameter} identifier
     * @return <code>true</code> if the {@link Parameter} store contains a
    ...
    /**
     * Removes all of the mappings from the {@link ParameterStore}. The
     * {@link ParameterStore} will be empty after this call returns.
     */
