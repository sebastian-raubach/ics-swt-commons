...
public class ShellUtils
{
     ...
     * 
     * @param shell
     *            The {@link Shell} to apply the new dimensions to
     */
     ...
     * <ul>
     * <li>x: The minimum of {@link #MIN_WIDTH} ({@value #MIN_WIDTH}px) and the
     * available screen width</li>
     * <li>y: The minimum of {@link #MIN_HEIGHT} ({@value #MIN_HEIGHT}px) and
     * the available screen height</li>
     * </ul>
     * 
     * @param shell
     */
    public static void setMinSize(Shell shell)
    {
        /* Get the current monitors dimensions */
        Rectangle screen = Display.getCurrent().getPrimaryMonitor().getBounds();

        /* Calculate width: MIN_WIDTH <= width <= screen.width */
        int width = Math.min(screen.width, MIN_WIDTH);
        /* Calculate height: MIN_HEIGHT <= height <= screen.height */
        int height = Math.min(screen.height, MIN_HEIGHT);
     ...
     * 
     * @param parent
     *            The parent {@link Shell} to center to
     * @param thisSize
     *            The size of the component to center
